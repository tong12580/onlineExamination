...
<html>
  <head>
    ...
    <title>questions2List.jsp</title>
	...
	$(function(){
				...
				buttons:[{		//添加对话框按钮
					...
					handler:function(){
						...
						$("#addQusetionsFormInfo").form('submit',{
							//将表单序列化，并传递
							url:'questions2ListAction?m=saveQusetionsInfo&'+$("#addQusetionsFormInfo").serialize(),
							onSubmit:function(){
		...
		//返回一条选中记录，并判断是否非空
			if($("#userDataGrid").datagrid('getSelected')==null){
				...
				//拿到要修改的表单
				$("#updateQusetionsFormInfo").form('load',"questions2ListAction?m=findQusetionsInfoById&fId="+$("#userDataGrid").datagrid('getSelected').id);				//URL加载				
				//弹出要修改的对话框
				...
				buttons:[{		//添加对话框按钮
					...
					handler:function(){
						...
						$("#updateQusetionsFormInfo").form('submit',{
							//将表单序列化，并传递
							url:'questions2ListAction?m=updateQuestionsInfo&'+$("#updateQusetionsFormInfo").serialize(),
							onSubmit:function(){
							//onsubmit事件调用表单validate方法判断 输入是否合法
		...
		$("#removechbut").click(function(){
			...
			if($("#userDataGrid").datagrid('getSelected')==null){
				...
				$.post("questions2ListAction?m=deleteQuestionsInfoById&dId="+$("#userDataGrid").datagrid('getSelected').id,function(data){
					if(data=='true'){
								$.messager.show({
											title:'消息提示',
											msg:'删除成功',
		...
		$("#userDataGrid").datagrid({
			fit:true,
			fitColumns:true,
			url:'questions2ListAction?m=findWithPage',
			pagination:true,
			pagePosition:'both',
			//表头添加属性
			toolbar:'#tb',
